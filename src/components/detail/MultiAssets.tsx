...
import AssetsQuantityPanel from '@components/detail/modules/AssetsQuantityPanel';
import AssetsOrderPanel from '@components/detail/modules/AssetsOrderPanel';
import AssetsOhterOwnerPanel from '@components/detail/modules/AssetsOhterOwnerPanel';
import OwnerLIstModal from '@components/detail/modals/OwnerLIstModal';
...
function MultiAssets({ nftAssets, ownerList, isFirst, handleMarketCancel }: AssetsDetail) {
    const { t } = useTranslation();
    const [isOwnerListModal, setOwnerListModal] = useState<boolean>(false);
    ...
    const setOrder = async (nft: IAssetsDetail) => {
        ...
        // 총 가격
        const totalPrice = new Decimal(nft.price).mul(quantity).toString();
        ...
        const catractPrams = [
            ...
            sellerPrice,
            nft.metadata_url,
            nft.market_quantity - quantity <= 0 || nft.owner_is_end_count <= 0,
        ];
        ...
        <>
            <StickyBox>
                <SingleContainer>
                    ...
                    {/* 멀티 다른 판매자 */}
                    <AssetsOhterOwnerPanel
                        ownerList={ownerList}
                        setOwnerListModal={setOwnerListModal}
                    />
